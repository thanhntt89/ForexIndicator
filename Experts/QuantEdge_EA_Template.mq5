...
// [ARROW-OWNERSHIP] Older builds of this EA drew their own signal arrows
// under this prefix. They are unmanaged now, so clear them out.
#define QEEA_LEGACY_ARROW_PREFIX "QEEA_Arr_"
void QEEA_CleanupLegacyArrows()
{
   int total = ObjectsTotal(0);
   for(int i = total - 1; i >= 0; i--)
   {
      string name = ObjectName(0, i);
      if(StringFind(name, QEEA_LEGACY_ARROW_PREFIX) == 0)
         ObjectDelete(0, name);
   }
}
...
// select. Two programs drawing the same marker under different prefixes
// only created two sets to keep in sync. Run the indicator with
...
//| far away. The chart showed one arrow and two trades, because the   |
//| arrow for that bar already existed and was not drawn twice.       |
//|                                                                   |
...
bool HasOpenPosition(int direction)
{
   ...
   ulong magicTP3 = InpMagicNumber + MAGIC_TP3_OFFSET;
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(mag != (long)InpMagicNumber && mag != (long)magicTP2 && mag != (long)magicTP3)
         continue;
...
bool IsOurMagic(long magic)
{
   ...
   if(magic == (long)(InpMagicNumber + MAGIC_TP2_OFFSET))  return true;
   if(magic == (long)(InpMagicNumber + MAGIC_TP3_OFFSET))  return true;
   if(magic >= (long)(InpMagicNumber + MAGIC_POS_DCA_OFFSET) &&
      magic <  (long)(InpMagicNumber + MAGIC_POS_DCA_OFFSET + 100))
      return true;
   if(magic >= (long)(InpMagicNumber + MAGIC_NEG_DCA_OFFSET) &&
      magic <  (long)(InpMagicNumber + MAGIC_NEG_DCA_OFFSET + 100))
      return true;
   return false;
}
...
int CountDCAPositions(int dcaType)
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      long mag = PositionGetInteger(POSITION_MAGIC);
      if(mag >= (long)(InpMagicNumber + dcaType) &&
         mag <  (long)(InpMagicNumber + dcaType + 100))
         count++;
   }
   return count;
}
...
bool HasAnyDCAPosition()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(PositionGetString(POSITION_SYMBOL) != Symbol()) continue;
      long mag = PositionGetInteger(POSITION_MAGIC);
      if(mag >= (long)(InpMagicNumber + MAGIC_POS_DCA_OFFSET) &&
...
//+------------------------------------------------------------------+
//| Sum floating P/L (profit + swap) for all positions of our EA     |
//+------------------------------------------------------------------+
...
double CalculateNegDCAAvgEntry()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      bool isOriginal = (mag == (long)InpMagicNumber || mag == (long)magicTP2 || mag == (long)magicTP3);
...
void ApplyDCABackstopSL()
{
   ...
   double distancePrice  = distancePoints * _Point * InpDCABackstopBufferMult;

   double backstopSL = (g_dcaDirection > 0)
                        ? NormalizeDouble(avgEntry - distancePrice, _Digits)
                        : NormalizeDouble(avgEntry + distancePrice, _Digits);
   ...
   if(g_dcaDirection > 0 && backstopSL >= bid - stoplevel)
      backstopSL = NormalizeDouble(bid - stoplevel - _Point, _Digits);
   if(g_dcaDirection < 0 && backstopSL <= ask + stoplevel)
      backstopSL = NormalizeDouble(ask + stoplevel + _Point, _Digits);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      CTrade bsTrade;
      bsTrade.SetExpertMagicNumber((ulong)mag);
      if(!bsTrade.PositionModify(ticket, backstopSL, curTP))
         Print("[QuantEdge EA] Backstop SL modify FAILED ticket=", ticket, ": ", bsTrade.ResultRetcodeDescription());
   }
}
...
//| real PC wall clock and on MT5 it tracks server time rather than   |
//| the bar being simulated. Every gate below that asks "what hour is |
//| it" therefore judged the WRONG moment during a backtest — run a   |
//| test overnight with the 07-20 session filter on and Gate 6        |
//| rejected all of it, which looks exactly like "the EA never        |
//| trades".                                                          |
//|                                                                   |
...
void QEEA_TimeGMT(MqlDateTime &dt)
{
   TimeToStruct(QEEA_TimeGMT(), dt);
}
...
void UpdateDailyLossTracking()
{
   ...
   if(today != g_dailyResetDate)      g_dailyResetDate   = today;
   if(weekStart != g_weeklyResetDate) g_weeklyResetDate  = weekStart;
   if(monthStart != g_monthlyResetDate) g_monthlyResetDate = monthStart;

   // Widest window (month) covers daily+weekly buckets too — single pass.
   HistorySelect(g_monthlyResetDate, TimeCurrent());
   ...
   for(int i = 0; i < totalDeals; i++)
   {
      ...
      if(dealTime >= g_weeklyResetDate) g_weeklyLossAmount += absLoss;
      if(dealTime >= g_dailyResetDate)
      {
         g_dailyLossCount++;
...
bool IsWeeklyDDStopHit()
{
   ...
   double maxLoss = AccountInfoDouble(ACCOUNT_BALANCE) * InpMaxWeeklyDDPct / 100.0;
...
bool IsMonthlyDDStopHit()
{
   if(!InpUseMonthlyDDStop || InpMaxMonthlyDDPct <= 0)
      return false;
   static bool s_wasHit = false;
   double maxLoss = AccountInfoDouble(ACCOUNT_BALANCE) * InpMaxMonthlyDDPct / 100.0;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(posType == POSITION_TYPE_BUY)
      {
         ...
         if(newSL > openPrice && newSL > curSL)
            trailTrade.PositionModify(ticket, newSL, curTP);
      }
...
bool IsDCASpacingOK(double currentPrice)
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(MathAbs(currentPrice - PositionGetDouble(POSITION_PRICE_OPEN)) < minDist)
         return false;
   }
   return true;
}
...
void ManagePositiveDCA()
{
   ...
   double atrBuf[1];
   if(g_hATR == INVALID_HANDLE || CopyBuffer(g_hATR, 0, 0, 1, atrBuf) != 1)
      return;
   double atrSpacing = atrBuf[0] * InpPosDCAATRMult;
   if(atrSpacing <= 0)
      return;

   for(int idx = 1; idx <= InpPosDCAMaxOrders; idx++)
   {
      ulong dcaMagic = InpMagicNumber + MAGIC_POS_DCA_OFFSET + idx;
      ...
      for(int i = PositionsTotal() - 1; i >= 0; i--)
      {
         ...
         if(ticket == 0) continue;
         if(PositionGetString(POSITION_SYMBOL) != Symbol()) continue;
         if(PositionGetInteger(POSITION_MAGIC) == (long)dcaMagic) { exists = true; break; }
      }
      if(exists) continue;
      ...
      if(result)
      {
         ...
         Print("[QuantEdge EA] Positive DCA+", idx, " FAILED: ",
               dcaTrade.ResultRetcodeDescription());
   }
}
...
bool CheckEntryReturnProfitClose()
{
   ...
   bool nearEntry = MathAbs(price - g_dcaOriginalEntry) <= MathMax(spread, _Point);
   if(!nearEntry)
      return false;
...
void ManageNegativeDCA()
{
      ...
      if(avgEntry > 0)
      {
         ...
         if(atBreakeven)
         {
            ...
            Print("[QuantEdge EA] EXIT: Negative DCA breakeven — avg=",
                  DoubleToString(avgEntry, _Digits), " target=",
                  DoubleToString(beTarget, _Digits), " basket P/L=",
   ...
   if(!g_dcaNegTriggered)
   {
      ...
      Print("[QuantEdge EA] Negative DCA TRIGGERED at price ",
            DoubleToString((g_dcaDirection > 0 ? ask : bid), _Digits),
            " (trigger level=", DoubleToString(triggerPrice, _Digits), ")");
   }
   ...
   if(g_hATR == INVALID_HANDLE || CopyBuffer(g_hATR, 0, 0, 1, atrBuf) != 1)
      return;
   double atrSpacing = atrBuf[0] * InpNegDCAATRMult;
   if(atrSpacing <= 0)
      return;
   ...
   {
      ulong dcaMagic = InpMagicNumber + MAGIC_NEG_DCA_OFFSET + idx;
   ...
   if(!g_dcaTP1HalfClosed && g_dcaOriginalTP1 > 0)
   {
      ...
      if(tp1Reached)
      {
         ...
         Print("[QuantEdge EA] EXIT: TP1 REACHED — TP1=",
               DoubleToString(g_dcaOriginalTP1, _Digits), " basket P/L=",
...
int OnInit()
{
   ...
   if(g_dcaActive)
      Print("[QuantEdge EA] DCA state restored: dir=", (g_dcaDirection > 0 ? "BUY" : "SELL"),
            " entry=", DoubleToString(g_dcaOriginalEntry, _Digits),
      ...
      if(tp1Hit)
      {
         ...
         if(InpInvalidateOnTP1)
         {
            Print("[QuantEdge EA] Retry: signal invalidated — TP1 reached (",
                  DoubleToString(g_sigTP1, _Digits), ") before entry");
            g_sigValid = false;
            return false;
         }
...
bool TryExecuteSignal(bool isRetry)
{
   ...
   // --- Validate signal data (reject garbage from indicator buffers) ---
   {
      ...
      {
         if(!isRetry)
            Print("[QuantEdge EA] Signal data not ready: entry=", DoubleToString(entry, _Digits),
                  " SL=", DoubleToString(sl, _Digits),
                  " TP1=", DoubleToString(tp1, _Digits),
                  " (TP dist=", DoubleToString(MathAbs(tp1 - entry) / _Point, 0), " pts",
                  " min=", DoubleToString(minTPDist / _Point, 0), " pts)",
   ...
   if(InpUseGate5Spread)
   {
      ...
      if(g5_pass && InpMaxSpreadPctOfTP1 > 0 && tp1Dist > 0)
      {
         double spreadPct = (spreadPts * _Point) / tp1Dist * 100.0;
   ...
   {
      adjSL  = NormalizeDouble(sl, _Digits);
      adjTP1 = NormalizeDouble(tp1, _Digits);
      adjTP2 = (tp2 != EMPTY_VALUE && tp2 > 0) ? NormalizeDouble(tp2, _Digits) : 0;
      adjTP3 = (tp3 != EMPTY_VALUE && tp3 > 0) ? NormalizeDouble(tp3, _Digits) : 0;
   }
   else
   {
      double priceShift = marketPrice - entry;
      adjSL  = NormalizeDouble(sl  + priceShift, _Digits);
      adjTP1 = NormalizeDouble(tp1 + priceShift, _Digits);
      adjTP2 = (tp2 != EMPTY_VALUE && tp2 > 0)
               ? NormalizeDouble(tp2 + priceShift, _Digits) : 0;
      adjTP3 = (tp3 != EMPTY_VALUE && tp3 > 0)
               ? NormalizeDouble(tp3 + priceShift, _Digits) : 0;
   }
   ...
   if(direction > 0)
   {
      ...
      if(adjTP3 > 0 && adjTP3 <= askPrice + stoplevel) adjTP3 = NormalizeDouble(askPrice + stoplevel + _Point, _Digits);
   }
   else
   {
      ...
      if(adjTP3 > 0 && adjTP3 >= bidPrice - stoplevel) adjTP3 = NormalizeDouble(bidPrice - stoplevel - _Point, _Digits);
   }
      ...
      if(riskLeft <= 0 || fillRR < InpMinFillRR)
      {
         ...
         if(!isRetry)
            Print("[QuantEdge EA] Gate 12 FAIL: remaining R:R=", DoubleToString(fillRR, 2),
                  ...
                  " SL=", DoubleToString(adjSL, _Digits),
   ...
   // late entry.
   double slDistance = MathAbs(marketPrice - adjSL) / _Point;
   ...
   if(lot <= 0)
   {
      Print("[QuantEdge EA] Lot calculation returned 0 — cannot trade.",
            ...
            " market=", DoubleToString(marketPrice, _Digits),
            " adjSL=", DoubleToString(adjSL, _Digits),
   ...
   if(!InpEnableAutoTrading)
   {
      ...
      Print("[QuantEdge EA] Would ", dirStr, " ", DoubleToString(lot, 2), " lot [", tpModeStr, "] @ ",
            DoubleToString(marketPrice, _Digits),
            " SL=", DoubleToString(adjSL, _Digits),
            " TP1=", DoubleToString(adjTP1, _Digits),
            " TP2=", DoubleToString(adjTP2, _Digits),
            " TP3=", DoubleToString(adjTP3, _Digits),
   ...
   if(InpUsePositiveDCA || InpUseNegativeDCA)
   {
      ...
      Print("[QuantEdge EA] DCA state initialized: entry=", DoubleToString(marketPrice, _Digits),
            " refSL=", DoubleToString(adjSL, _Digits),
...
void OnDeinit(const int reason)
{
   ...
   QEEA_DeletePanel();
   // [ARROW-OWNERSHIP] The EA no longer draws signal arrows — the indicator
   // owns them. Sweep any QEEA_Arr_ objects left behind by an older build so
   // a chart upgraded in place does not keep showing a second, now-orphaned
   // set of markers that nothing maintains. Unconditional: these are stale
   // whatever the deinit reason.
   QEEA_CleanupLegacyArrows();
