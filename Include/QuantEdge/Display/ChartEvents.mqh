                      ...
                      const double &dparam, const string &sparam)
{
   ...
   if(id == CHARTEVENT_OBJECT_CLICK && InpDashboardMode == DASHBOARD_MANUAL)
   {
      if(StringFind(sparam, PREFIX_CLOSE) == 0)
      {
         ...
         if(criteria >= 0)
         {
            // NOTE: MessageBox() is not usable from a custom indicator's
            // OnChartEvent on MQL5 (silently fails, short-circuiting the
            // command before it's sent) — fire the close command directly,
            // same as QuantEdge_EA_Template's own panel buttons.
